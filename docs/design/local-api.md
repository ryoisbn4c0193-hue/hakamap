...
- 成功レスポンスには必要な場合だけ`Cache-Control: no-store`を付け、ブラウザキャッシュへ
  Projectデータを残さない。`GET /api/v1/session`では必ず設定する。
...
- トークンを再取得する専用APIは設けず、認証済みの`GET /api/v1/session`の
  ...
  正しいHost、および同一オリジン要求であることの検証を要求する。
- `GET /api/v1/session`でOriginヘッダーが存在する場合は現在のオリジンとの完全一致を
  要求する。Originがない場合は、正しいHost、有効なセッションCookie、および
  `Sec-Fetch-Site: same-origin`または同一オリジンのRefererを要求する。
- 外部Origin、`Sec-Fetch-Site: cross-site`、不正Host、および判定材料がない要求を拒否する。
- `GET /api/v1/session`へ`Cache-Control: no-store`を必ず設定し、セッション再生成後に
  以前のCSRFトークンを含むレスポンスをブラウザキャッシュから再利用させない。
...
- 復旧APIの`action`は`apply | discard`だけを許可する。`apply`は正式JSONを直ちに
  ...
  `discard`は復旧適用用の完全性検証とは分けて、復旧JSONに記録された一時パスが
  対象Projectの一時領域内にあり、アセットUUIDと許可拡張子に一致することを検証する。
  検証済みの各パスを`deleteIfExists`相当で冪等に削除し、全対象が存在しないことを
  確認できた後だけ対応する復旧JSONを削除する。途中で削除に失敗した場合は他の対象の
  削除を続け、復旧JSONを維持して失敗として応答し、同じ破棄操作を再試行可能にする。
...
- `BackupRestoreOperationRequest`は後述のバックアップ一覧で発行した候補を使用し、
  現在開いている同じProjectを対象とする。
...
`BackupListResponse`はProject UUID、取得時の`revision`、および`items[]`を含む。
各`BackupListItemResponse`は次を含む。

```text
backupId
backupType = automatic | preRestore
createdAt
sizeBytes
applicationVersion?
projectName?
...
- 一覧取得時に`backup/automatic`と`backup/pre-restore`を走査し、通常ファイル、ZIP構造、
  マニフェスト、Project UUID、必要ファイル、および対応形式を安全に検証する。
- `backupId`は内部ファイル名やパスから推測できない不透明値とし、HTTPセッション内の
  検証済みバックアップ候補へ5分間だけ紐づける。
- `backupVersion`も不透明値とし、一覧取得時のファイルサイズ、更新情報、および内容SHA-256へ
  サーバー側で紐づける。ファイル名、絶対パス、SHA-256自体をレスポンスへ返さない。
- `createdAt`はマニフェストのUTC日時、`sizeBytes`は0以上の整数とする。
- 検証できたメタデータだけを`applicationVersion`と`projectName`へ返し、読み取れない場合は省略する。
- `restorable = false`の項目も件数把握と案内のため返してよいが、復元操作を有効にしない。
- `unavailableReason`は`corrupted`、`missingFiles`、`unsupportedVersion`、
  `projectMismatch`、`temporarilyUnavailable`の安全な固定分類だけを返す。
- 内部ファイル名、絶対パス、例外詳細、およびアセット名を返さない。
- 一覧応答へ`Cache-Control: no-store`を設定する。

`BackupRestoreOperationRequest`は次を含む。
...
- `confirmedNoUnsavedChanges`が`true`でない要求を拒否し、サーバーでもProjectの`dirty`が
  `false`であることを再確認する。クライアント申告だけを信頼しない。
- `expectedRevision`と現在のrevisionが一致しない場合は復元前退避も開始しない。
- 復元開始直前にバックアップを再走査してサイズ、更新情報、SHA-256、形式、および完全性を
  検証し、`backupVersion`の候補と一致しない場合は差し替えとして拒否する。
- 期限切れ、別セッション、一覧にない、利用不能、または差し替えられたバックアップでは
  Operationを開始せず、一覧を再取得するよう案内する。
...
| `backup-*` | バックアップ一覧、利用可否、差し替え、復元前提 |
...
| 404 | `backup-not-found` | バックアップ候補がない、期限切れ、または別セッション |
| 409 | `backup-version-conflict` | 一覧取得後にバックアップ実体が差し替えられた |
| 422 | `backup-unavailable` | 破損、欠落、非対応などのため復元できない |
| 409 | `backup-project-dirty` | 未保存変更があるため復元を開始できない |
...
- Originあり・なしの正常な同一オリジン要求で`GET /api/v1/session`を取得でき、
  外部Origin、cross-site、不正Host、および判定不能要求が拒否されることを確認する。
- `GET /api/v1/session`が`Cache-Control: no-store`を返し、セッション再生成後に
  以前のCSRFトークンを含む応答がキャッシュから再利用されないことを確認する。
- バックアップ一覧が安全なメタデータ、不透明ID、復元可否、理由分類を返し、内部名と
  絶対パスを返さないことを確認する。
- 復元開始時にrevision、未保存変更、候補期限、利用可否、および`backupVersion`を再検証し、
  一覧取得後に差し替えられたバックアップを復元しないことを確認する。
